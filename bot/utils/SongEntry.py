...
class SongEntry:
	...
	def to_rest_dict(self):
		return {'videoId': self.id, 'uploader': self.uploader, 'title': self.title, 'requester': self.requester, 'duration': self.duration, 'timestamp': math.floor(time.time())-self.play_start}